...
# Function to download file with wget
download_file() {
    local url="$1"
    local destination="$2"
    local filename="$3"
    ...
    if [ -f "$filepath" ]; then
        print_info "File already exists: $filename - skipping"
        return 0
    fi
    ...
    if wget -q --show-progress --progress=bar:force:noscroll --content-disposition "$url" -O "$filepath"; then
...
clone_extension() {
    ...
    if [ -d "$target_path" ]; then
        print_info "Extension already exists: $repo_name - pulling latest"
        (cd "$target_path" && git pull 2>/dev/null || echo "Failed to pull")
    else
...
main() {
    ...
    echo -e "${GREEN}All models and extensions have been downloaded successfully!${NC}"
    echo -e "${YELLOW}Total time: $(($SECONDS / 60)) minutes and $(($SECONDS % 60)) seconds${NC}"
}
